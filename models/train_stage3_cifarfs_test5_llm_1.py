...
class SemanticGuidedKnowledgeDistillation(nn.Module):
    ...
    def forward(self, student_logits, teacher_logits, current_class_indices=None, epoch=None, max_epoch=None):
        ...
        if self.similarity_matrix is not None and current_class_indices is not None:
            ...
            # �� 修复：确保progress和progress_factor总是被定义
            progress = 0.0
            progress_factor = 1.0
        
            # 基于训练进度的权重调整
            if epoch is not None and epoch <= 100:  # 前100个epoch强制低权重
                forced_weight = 0.05
                current_weight = forced_weight
                progress = epoch / max_epoch if max_epoch is not None else 0.0
                progress_factor = 0.05  # 强制低权重对应的factor
            
                if epoch % 10 == 0:
                    print(f"�� [EIRM-Fix] Epoch {epoch}: Forced semantic weight = {forced_weight}")
                
            elif epoch is not None and max_epoch is not None:
                # 原来的权重调度逻辑
                progress = epoch / max_epoch
            
                if progress < 0.3:  # 前30%的训练保持低权重
                    progress_factor = 0.3
                else:
                    progress_factor = 0.3 + 0.7 * (progress - 0.3) / 0.7
            
                current_weight = current_weight * progress_factor
        
            # �� 修复：现在progress和progress_factor总是有值
            if epoch is not None and epoch % 10 == 0:  # 每10个epoch打印一次
                print(f"�� [EIRM-SemanticKD] Epoch {epoch}: Progress={progress:.3f}, "
                      f"ProgressFactor={progress_factor:.3f}, SemanticWeight={current_weight:.4f}")
            # 4. 组合损失
...
if __name__ == '__main__':
    ...
    parser.add_argument('--irm-coef', type=float, default=0.1, help='Coefficient for IRM loss in total loss')
