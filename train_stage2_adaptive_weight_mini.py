...
def training(args):
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        tl, ta, losses, weights  = train(args, model, train_loader, optimizer)
        cls_loss, ss_loss, recon_loss, total_loss = losses
        cls_weight, ss_weight, recon_weight = weights
        lr_scheduler.step()
        ...
        print(f'Classification: {cls_weight:.4f}')
        print(f'Self-supervised: {ss_weight:.4f}')
        print(f'Reconstruction: {recon_weight:.4f}')
        print(f'IRM weight: 0.3000')

        print('\nCurrent loss values:')
        print(f'Classification: {cls_loss:.4f}')
        print(f'Self-supervised: {ss_loss:.4f}')
...
def train(args, model, train_loader, optimizer):
    ...
    for i, batch in enumerate(train_loader, 1):
        ...
        with autocast(device_type='cuda', dtype=torch.float16):
            ...
            current_weights = F.softplus(model.adaptive_weight.weights)
            ...
            final_weights = (
                current_weights[0].item(),
                current_weights[1].item(),
                current_weights[2].item()
            )
        tl.add(loss.item())
        ta.add(acc)
    ...
    return tl.item(), ta.item(), final_losses, final_weights
